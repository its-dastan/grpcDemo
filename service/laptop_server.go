...
func (server *LaptopServer) RateLaptop(stream pb.LaptopService_RateLaptopServer) error {
	for {
		...
		log.Printf("received a rate-laptop request with id = %s, score = %.2f", laptopId, score)
		...
		if err != nil {
			return logError(status.Errorf(codes.Unknown, "cannot add rating to the store: %v", err))
		}
		...
		err = stream.Send(res)
		if err != nil {
			return logError(status.Errorf(codes.Unknown, "cannot receive stream response : %v", err))
		}
	}
	return nil
}
